let filter=(brand,mcamera,displaySize,features,networkT,ram,min,max,mobiles)=>{
    ...
    if(network.length==0)
       ifWholeEmpty(networkT,network);   
    

       result=fillInResult(result,rams,sizes,camera,brands,min,max,mobiles,NFC,audioJack,IR,network);
...
let fillInResult=(result,rams,sizes,mcamera,brands,min,max,mobiles,NFC,audioJack,IR,network)=>{
     
    for(mobile of mobiles){
        if((mobile.price>min && mobile.price<max) && brands.includes(mobile.brand.toLowerCase().trim()) && rams.includes(mobile.memory.ram) && network.includes(mobile.network.category)){
            if(sizes.length!=0){
    ...
    if(NFC==true){
        ...
        result=result.filter((mobile)=>{
              if(mobile.network.nfc=="yes")
                return mobile;
        })
    } 
    if(IR==true){
        result=result.filter((mobile)=>{
            if(mobile.network.infrared=="yes")
              return mobile;
      })
    }
    if(audioJack==true){
        result=result.filter((mobile)=>{
            if(mobile.network.audioJack=="yes" || mobile.network.audioJack=="3.5mm")
              return mobile;
      })
    }
